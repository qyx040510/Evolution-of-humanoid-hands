...
gym.register(
    id="Isaac-ShadowHand-Grasp-v0", # 任务名称要保持唯一
    entry_point="isaaclab_tasks.evolution_tasks.task_grasp.shadowhand_grasp_env:GraspHandEnv",  # 定义任务（包括任务目标，奖励啥的，重置啥的）
    disable_env_checker=True, # 禁用环境检查器
    kwargs={
        # 定义对象
        "env_cfg_entry_point": "isaaclab_tasks.evolution_tasks.task_grasp.shadowhand_grasp_env_cfg:ShadowHandEnvCfg",
...
gym.register(
    id="Isaac-EvolutionHand-Grasp-v0", # 任务名称要保持唯一
    entry_point="isaaclab_tasks.evolution_tasks.task_grasp.evolution_grasp_env:EvolutionGraspEnv",  # 定义任务（包括任务目标，奖励啥的，重置啥的）（否）判断是否需要在——isaaclab_other中随着agent更新
    disable_env_checker=True, # 禁用环境检查器
    kwargs={
        # 定义对象
        "env_cfg_entry_point": "isaaclab_tasks.evolution_tasks.task_grasp.evolution_grasp_env_cfg:EvolutionGraspEnvCfg",# 需要根据agent更新
        # 提供rl_games 库的PPO（近端策略优化）算法的配置
        # 包括胜利条件，迭代Isaac-ShadowHand-StoneGrind-v0次数，多少次储存，batchsize等
        "rl_games_cfg_entry_point": f"{agents.__name__}:rl_games_ppo_cfg.yaml",
        # rl的PPO算法的参数配置，这个好像是对不同观察方式（如视觉等）来讲
